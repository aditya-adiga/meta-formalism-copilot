...
- `components/panels/` — One component per panel: `InputPanel`, `SemiformalPanel`, `LeanPanel`, `GraphPanel`, `NodeDetailPanel`, `CausalGraphPanel`, `StatisticalModelPanel`, `PropertyTestsPanel`, `BalancedPerspectivesPanel`, `CustomArtifactPanel`, `AnalyticsPanel`
...
- `lib/corpus/` — DD-009 corpus filesystem layer (sub-tasks S1+S2+S3, integrated). **S1:** `CorpusFS` interface + error kinds (`types.ts`), folder-layout path builders (`paths.ts` — **not** `layout.ts`, a reserved Next.js filename), `workspace.json` codec (`manifest.ts`), OPFS adapter (`opfsAdapter.ts`), default-off production-guarded dev flag (`flag.ts`), and the store storage-seam selector (`storeAdapter.ts`). **S2:** the opt-in user-visible FSA folder mirror — an FSA-backed `CorpusFS` (`fsaAdapter.ts`), a composite that mirrors OPFS→FSA async-with-retry and reads OPFS→FSA fallthrough while exposing a truthful sync-ack via `getMirrorStatus()`/`onMirror()` (`mirrorFs.ts`), and the folder-pick + permission + handle-persistence flow (`fsaPicker.ts`); when a folder is connected, `storeAdapter.resolveCorpusFs()` makes the store's `CorpusFS` the mirror composite. **S3:** the isomorphic-git commit/push/pull pipeline as a SEPARATE `CorpusGit` interface that operates *over* a `CorpusFS` (NOT methods on it — ISP) — an iso-git→`CorpusFS` shim (`gitFs.ts`), the testable git logic over injected `{fs,http,dir,onAuth}` (`gitCore.ts` — real `isomorphic-git`; lazy-paginated `log` via `{depth,cursor}`; conflict→`git-conflict`, auth-fail→`remote-auth-expired` typed errors), the typed worker protocol + reifying dispatch (`gitProtocol.ts`), the main-thread `CorpusGit` proxy over an injected transport (`gitWorkerClient.ts`), and the thin dedicated Web Worker entry (`gitWorker.ts`); adds the `isomorphic-git` + `buffer` deps (contained to the worker-side files). Everything is behind the flag; localStorage is still the default persistence path; migration + commit-on-write + the "saved" status UI are deferred to S4/S5. Note for tests: **jsdom has no OPFS/FSA/Worker/network**, so adapter and git success paths are verified via in-memory/fake-handle fakes + real iso-git over an in-memory `CorpusFS` (vitest runs on Node) + the shared `CorpusFS` contract; real-browser/worker/remote checks are deferred to out-of-CI smokes (`docs/spikes/corpus-opfs-smoke.md`, `corpus-fsa-smoke.md`, `corpus-git-smoke.md`).
...
- **Multiple artifact types**: semiformal, lean, causal-graph, statistical-model, property-tests, balanced-perspectives — each with a dedicated panel and API route. Users can also create **custom artifact types** with user-authored system prompts, generated via a generic `/api/formalization/custom` route.
