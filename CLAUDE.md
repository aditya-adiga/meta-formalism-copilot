...
- `lib/corpus/` — DD-009 corpus filesystem layer (sub-tasks S1+S2): `CorpusFS` interface + error kinds (`types.ts`), folder-layout path builders (`paths.ts` — **not** `layout.ts`, which is a reserved Next.js filename), `workspace.json` codec (`manifest.ts`), OPFS adapter (`opfsAdapter.ts`), default-off dev flag (`flag.ts`), and the store storage-seam selector (`storeAdapter.ts`). **S2** adds the opt-in user-visible FSA folder mirror: an FSA-backed `CorpusFS` (`fsaAdapter.ts`), a composite that mirrors OPFS→FSA async-with-retry and reads with OPFS→FSA fallthrough while exposing a truthful sync-ack via `getMirrorStatus()`/`onMirror()` (`mirrorFs.ts`), and the folder-pick + permission + handle-persistence flow (`fsaPicker.ts`). When a folder is connected, `storeAdapter`'s `resolveCorpusFs()` makes the store's `CorpusFS` the mirror composite instead of bare OPFS. Behind a flag; localStorage is still the default persistence path. Note for tests: **jsdom has neither OPFS nor FSA**, so adapter success paths are verified via in-memory/fake-handle fakes + the shared `CorpusFS` contract, with real-browser checks deferred to out-of-CI smokes (`docs/spikes/corpus-opfs-smoke.md`, `docs/spikes/corpus-fsa-smoke.md`).
