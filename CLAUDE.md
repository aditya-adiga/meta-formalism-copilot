...
- `lib/corpus/` — DD-009 corpus filesystem layer (sub-tasks S1+S3): `CorpusFS` interface + error kinds + the `CorpusGit` interface (`types.ts`), folder-layout path builders (`paths.ts` — **not** `layout.ts`, which is a reserved Next.js filename), `workspace.json` codec (`manifest.ts`), OPFS adapter (`opfsAdapter.ts`), default-off dev flag (`flag.ts`), and the store storage-seam selector (`storeAdapter.ts`). **S3** adds the isomorphic-git commit/push/pull pipeline, which is a SEPARATE `CorpusGit` interface that operates *over* a `CorpusFS` (NOT methods on `CorpusFS` — ISP): an iso-git→`CorpusFS` shim (`gitFs.ts`), the plain testable git logic over injected `{fs,http,dir,onAuth}` (`gitCore.ts` — real `isomorphic-git`; lazy-paginated `log` via `{depth,cursor}` to dodge the 3000-commit cliff; conflict→`git-conflict`, auth-fail→`remote-auth-expired` typed errors), the typed worker request/response protocol + reifying dispatch (`gitProtocol.ts`), the main-thread `CorpusGit` proxy over an injected transport (`gitWorkerClient.ts`), and the thin dedicated Web Worker entry that runs iso-git off the main thread with a worker-scope `Buffer` polyfill (`gitWorker.ts` — named to avoid Next.js reserved filenames). Adds the `isomorphic-git` + `buffer` dependencies (the initiative's first runtime deps; iso-git is contained to the worker-side files). Behind a flag; localStorage is still the default persistence path; commit-on-write is deferred to S4. Note for tests: **jsdom has no Worker/OPFS/network**, so the git LOGIC is tested with real iso-git against an in-memory `CorpusFS` (vitest runs on Node) and the typed-error boundary over a fake transport; the real worker + real OPFS + real remote push/pull are deferred to an out-of-CI smoke (`docs/spikes/corpus-git-smoke.md`).
