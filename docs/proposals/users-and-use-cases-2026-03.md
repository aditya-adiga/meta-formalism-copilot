...
### Proposed features: New artifact categories

| # | Feature | Groups served | Description |
|---|---------|---------------|-------------|
| F12 | Template-matching formalisms | E | A new category of artifact type where the "formalization" maps source material onto a pre-existing structural framework. Examples: Campbell's monomyth, Toulmin's argument model, Porter's Five Forces, Lakatos's research programmes. The pluralism angle is a direct fit — the *same* material through different templates reveals different structure. |
| F13 | Audience-targeted recomposition | D | Inverts the tool's current direction. Takes a structured decomposition (any artifact type) and repackages it for a specific audience or format: TED talk structure, executive summary, classroom explanation, blog post. Formal → accessible instead of informal → formal. |
...
| F22 | Alternative formalization discovery | A, C, E | After producing a formalization, surface how the same concept has been formalized differently elsewhere. "Your insight was formalized using category theory, but there's also an order-theoretic approach in [paper X]." The most philosophically aligned sub-capability — it directly operationalizes "generalization via inclusion" through external knowledge. Connects to F12 (template-matching): if templates exist, this feature can suggest "you used Toulmin, but pragma-dialectics highlights different structure in similar arguments." |
...
### Proposed features: Enhanced output and comparison

| # | Feature | Groups served | Description |
|---|---------|---------------|-------------|
| F14 | Comparative lens view | A, C, E | Side-by-side display of the same source material through 2-3 artifact types simultaneously. Directly tests the core "generalization via inclusion" thesis — does seeing causal graph + dialectical map + counterexamples together produce insight that any single view doesn't? Related to F22 (alternative formalizations) but uses the tool's *own* artifact types rather than external knowledge. |
...
| F18 | Framework library | E | Curated set of structural templates (narrative, strategic, argumentative) users can select or define custom ones. The template defines the expected structure; the tool maps source material onto it. F22 can dynamically suggest frameworks from this library. |
...
| B | **Source material contains propositional content** — claims that can be decomposed into truth-apt statements | Hard | Template-matching formalisms (F12) relax this to "source material has mappable structure," which is broader. |
...
| E1. Narrative analyst | ✓ | ✗‡ | ✓ | ✓ | ✓ | ✓ | Low‖ | Moderate (needs F12) |
| E2. Strategist | ✓ | ✗‡ | ✓ | ✓ | ✓ | ✓ | Low‖ | Moderate (needs F12) |
| E3. Argument mapper | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | Moderate (F22‖) | **Strong** (needs F12) |
...
‡ Source material has *structure* but not *propositions*; needs F12 (template-matching) to fit.
...
| Template-match fidelity | Does the structural mapping actually follow the framework correctly? | Expert review against framework definition (e.g., does the Campbell mapping correctly identify the "threshold crossing"?) | E |
...
3. **Framework plurality test** — For template-matching formalisms (F12): give users the same material mapped onto 2-3 different frameworks. Can they articulate what each framework reveals that the others don't? Do they develop a richer understanding than users who saw only one mapping?
...
- **F14 (comparative lens view)** — UI-level change, no new artifact generation needed. Directly tests the core thesis and serves the strongest-fit archetypes.

### High impact, moderate effort — knowledge grounding (literature search)

Literature search capabilities (F19-F23) enhance the existing workflow rather than expanding input/output modalities. They serve the same function as the comparative lens view (F14) but through *external* knowledge rather than the tool's own artifact types. See [feature-proposals-2026-02.md §5](feature-proposals-2026-02.md) for the full design space exploration.
...
- **F22 (alternative formalizations)** — The philosophical keystone. Connects literature search to the core "generalization via inclusion" thesis. Also the bridge to F12 (template-matching): once frameworks exist, F22 can dynamically suggest which ones are relevant. Highest potential impact but requires deep semantic understanding.
...
Recommended sequence: F19 → F20 → F21 → F23 → F22. Each builds on the search infrastructure of the previous one. F22 is last because it's most ambitious and benefits from all the others being in place.

### High impact, significant effort — new artifact categories

- **F12 (template-matching formalisms)** — Opens the entirely new Group E. Requires designing a framework specification format and deciding how much the tool should "know" about each framework vs. letting users define custom ones. Arguably the most *novel* direction — no existing tool does "map this onto Campbell AND Toulmin AND a causal graph and compare." F22 (alternative formalizations) becomes much more powerful once this exists — it can suggest relevant frameworks dynamically.
- **F13 (audience recomposition)** — Inverts the tool's direction (formal → accessible). Opens Group D. Changes the pitch from "make your thinking rigorous" to "make your thinking rigorous *and then communicable*."
...
- **F17 (domain-specific export)** — Makes output actionable outside the tool. Lean project files, R scripts, graph formats. Pairs with F23 (living bibliography) for academic users.
- **F18 (framework library)** — Prerequisite for F12 if the template-matching direction is pursued. Could start as a curated list of 5-10 well-known frameworks. F22 can search this library to suggest relevant frameworks.
...
2. **Is template-matching a "formalization"?** Mapping a story onto Campbell's monomyth is a kind of structural analysis, but it's not truth-apt in the same way that a semiformal proof is. Does the tool's identity stretch to include this, or is it a different tool?

3. **How important is the recomposition direction (F13)?** It doubles the tool's scope (informal→formal *and* formal→accessible) but risks diluting the core focus. On the other hand, "rigor that stays locked in a tool" is less valuable than "rigor that reaches an audience."

4. **What's the minimum viable evaluation?** A full comparative study is expensive. Could we start with something lighter — e.g., a structured self-report from 5 users: "paste something you wrote, use 3 artifact types, tell us what you learned"?

5. **Should non-text input (F10, F11) change the decomposition step?** Currently decomposition extracts propositions from text. For data, you'd extract relationships and patterns. For code, you'd extract contracts and invariants. Is this the same pipeline with different extractors, or a fundamentally different workflow?

6. **How should literature search generalize beyond academic papers?** F19-F23 are scoped to academic literature and the Lean ecosystem, which leaves Groups E and F underserved. The underlying capability — "suggest alternative structural interpretations" — applies to framework catalogs, methodological pattern libraries, case law databases, and more. Should this be a single extensible search system with pluggable backends, or separate features for different knowledge domains?

7. **Where does literature search live in the UI?** Three options with different tradeoffs: (a) a dedicated Literature panel — visible but adds navigation weight; (b) integrated into existing panels (Mathlib hints in the Lean panel, paper suggestions in the context panel) — seamless but scattered; (c) a persistent sidebar element that accumulates references across panels — ambient but potentially noisy. The answer may differ by sub-capability (F19 fits naturally in the Lean panel; F23 wants its own space).
